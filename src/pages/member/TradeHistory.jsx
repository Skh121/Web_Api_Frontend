...
import Header from "../../components/admin/Log Management/Header";
...
// Main Component
const TradeHistory = () => {
  const { data: trades, isLoading, isError, error } = useGetTrades();
  ...
  const calculatePL = (trade) => {
    ...
    if (trade.tradeDirection.toLowerCase() === 'long') {
        return exitValue - entryValue - fees;
    } else { // short
        return entryValue - exitValue - fees;
    }
  };
    ...
    if (!trades || trades.length === 0) {
      return (
        <div className="text-center py-10 bg-gray-800 rounded-lg">
          ...
          <Link to="/admin/log-trade" className="mt-4 inline-block bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg hover:bg-blue-700 transition-colors">
      ...
      <div className="overflow-x-auto rounded-lg border border-gray-700">
        <table className="min-w-full text-sm text-left text-gray-300">
          <thead className="text-xs text-gray-400 uppercase bg-gray-700">
            <tr>
              <th scope="col" className="px-4 py-3">Chart</th>
          ...
          <tbody className="bg-gray-800">
            {trades.map((trade) => {
              ...
              const directionColor = trade.tradeDirection.toLowerCase() === 'long' ? 'text-green-400' : 'text-red-400';
              return (
                <tr key={trade._id} className="border-b border-gray-700 hover:bg-gray-700/50">
                  <td className="px-4 py-3">
                    {trade.chartScreenshotUrl ? (
                      <a href={trade.chartScreenshotUrl} target="_blank" rel="noopener noreferrer">
                        <img
                  ...
                  <td className={`px-6 py-4 font-semibold ${directionColor}`}>{trade.tradeDirection.toUpperCase()}</td>
                  <td className="px-6 py-4">{new Date(trade.entryDate).toLocaleDateString()}</td>
                  <td className={`px-6 py-4 font-bold ${plColor}`}>{pl.toFixed(2)}</td>
                  <td className="px-6 py-4 flex items-center gap-4">
                    <Link to={`/log/edit/${trade._id}`} className="text-blue-500 hover:text-blue-400 text-lg"><FaEdit /></Link>
                    <button 
  ...
  return (
    <div className="bg-gray-900 text-gray-200 min-h-screen p-4 sm:p-6 md:p-8">
      <div className="max-w-7xl mx-auto">
        {/* --- 2. THE OLD HEADER IS REPLACED WITH YOUR HEADER COMPONENT --- */}
        <Header />
        
        {/* We add some margin-top here for spacing below the new header */}
        <div className="mt-8">
            {renderContent()}
        </div>
      </div>
