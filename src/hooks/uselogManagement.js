...
import {
  ...
  updateTradeService,
  deleteTradeService,
  getTradesForReportService
} from "../services/admin/logManagementService";
...
export const useGetTrades = (page, limit = 5) => {
  return useQuery({
    queryKey: ["trades", page],
    queryFn: () => getTradesService({ page, limit }),
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["trades", variables.tradeId],
      });
      navigate("/trades");
    },
    onError: (err) => {
...
export const useGenerateReport = () => {
  return useMutation({
    mutationFn: getTradesForReportService,
    onError: (err) => {
      toast.error(
